...
dirPath <- paste0(constants$rootPath,"results/50_",constants$maxIter,"_",constants$populationSize)
...
myfitness <- function(individual, targetUser, feature.names){
  ...
  p <- probs[which(colnames(probs) == targetUser)]
  return(p)
}
...
for(targetUser in targetUsers){
  ...
  #prediction <- as.character(predict(m, newdata = adv, type = "class"))
